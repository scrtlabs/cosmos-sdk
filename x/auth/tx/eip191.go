...
func (s signModeEIP191Handler) GetSignBytes(mode signingtypes.SignMode, data signing.SignerData, tx sdk.Tx) ([]byte, error) {
	...
	if protoTx.txBodyHasUnknownNonCriticals {
		return nil, sdkerrors.ErrInvalidRequest.Wrap(eip191NonCriticalFieldsError)
	}

	body := protoTx.tx.Body

	if len(body.ExtensionOptions) != 0 || len(body.NonCriticalExtensionOptions) != 0 {
		return nil, sdkerrors.ErrInvalidRequest.Wrap("SignMode_SIGN_MODE_EIP_191 does not support protobuf extension options.")
	}
