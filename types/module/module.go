...
func (m *Manager) InitGenesis(ctx sdk.Context, cdc codec.JSONCodec, genesisData map[string]json.RawMessage) (*abci.ResponseInitChain, error) {
	...
	if len(validatorUpdates) == 0 {
		return &abci.ResponseInitChain{}, fmt.Errorf("validator set is empty after InitGenesis, please ensure at least one validator is initialized with a delegation greater than or equal to the DefaultPowerReduction (%d)", sdk.DefaultPowerReduction.Uint64())
	}
...
func (m Manager) RunMigrations(ctx context.Context, cfg Configurator, fromVM VersionMap) (VersionMap, error) {
	...
	for _, moduleName := range modules {
		...
		if exists {
			sdkCtx.Logger().Info(fmt.Sprintf("Running %s migration...", moduleName))
